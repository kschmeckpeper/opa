...
class Hourglass(nn.Module):
    def __init__(self, n, in_channels, out_channels, n_features=256):
        super(Hourglass, self).__init__()
        self.up1 = Residual(in_channels, n_features)
        self.up2 = Residual(n_features, n_features)
        self.up4 = Residual(n_features, out_channels)

        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.low1 = Residual(in_channels, n_features)
        self.low2 = Residual(n_features, n_features)
        self.low5 = Residual(n_features, n_features)
        if n > 1:
            self.low6 = Hourglass(n-1, n_features, out_channels)
        else:
            self.low6 = Residual(n_features, out_channels)
